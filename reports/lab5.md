...
因为不涉及多处理器，所以关了中断就是线性执行，代码简单了许多。
...
需求矩阵岂不是要看看代码实现？分配矩阵岂不是要从一开始就记录才能得到？
所以关键是需求矩阵...但如果能分析代码得到需求矩阵，岂不是不允许代码就能知道是否产生死锁...
死锁检测也就是编译器的事了

学习代码： https://github.com/LearningOS/2024a-rcore-silver-ymz/blob/ch8/os/src/syscall/sync.rs

误解：死锁不是上锁失败，是两个以上运行单元相互等待，谁也无法前进一步
如此说来，银行家算法则是可行的。
需求是上锁前试探性添加。

死锁检测处理这样的情况：🔒可以拿到，但接下来就是死锁。
